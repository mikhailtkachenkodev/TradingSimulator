...
orders_log_path = output/orders.csv
    ...
    std::string ModifyConfigValue(const std::string& base,
    ...
    std::string RemoveSection(const std::string& base, const std::string& section) {
        std::string result = base;
    ...
    // Helper: Verify default config values
    void VerifyDefaultConfig(const Config& config) {
        EXPECT_EQ(config.initial_price, 100);
        EXPECT_DOUBLE_EQ(config.average_trend_value, 0.05);
        EXPECT_DOUBLE_EQ(config.price_variation, 0.10);
        EXPECT_EQ(config.time_horizon, 24h);
        EXPECT_EQ(config.min_diff_time, 100ms);
        EXPECT_EQ(config.max_diff_time, 200ms);
        EXPECT_EQ(config.fast_ema, 1s);
        EXPECT_EQ(config.slow_ema, 5s);
        EXPECT_EQ(config.min_volume, 1);
        EXPECT_EQ(config.max_volume, 1000);
        EXPECT_EQ(config.min_position, -1000);
        EXPECT_EQ(config.max_position, 1000);
        EXPECT_DOUBLE_EQ(config.rejection_probability, 1.0);
        EXPECT_EQ(config.steps_count, 100000);
        EXPECT_EQ(config.price_evolution_path, "output/price_evolution.csv");
        EXPECT_EQ(config.orders_log_path, "output/orders.csv");
    }
};
...
TEST_F(ConfigManagerTest, CreateDefaultConfigSuccess) {
    ...
    ASSERT_TRUE(std::filesystem::exists(test_config_path)) << "Config file should exist";
    VerifyDefaultConfig(*result);
    ...
    VerifyDefaultConfig(*loaded);
}

TEST_F(ConfigManagerTest, ParseDurationNanoseconds) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "min_diff_time", "1ns"));
...
TEST_F(ConfigManagerTest, ParseDurationMicroseconds) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "min_diff_time", "500us"));
...
TEST_F(ConfigManagerTest, ParseDurationMilliseconds) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "min_diff_time", "100ms"));
...
TEST_F(ConfigManagerTest, ParseDurationMinutes) {
    ...
    config = ModifyConfigValue(config, "slow_ema", "10min");  // Must be > fast_ema
    WriteConfigFile(config);
...
TEST_F(ConfigManagerTest, ParseDurationHours) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "time_horizon", "24h"));
...
TEST_F(ConfigManagerTest, ParseDurationDays) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "time_horizon", "7d"));
...
TEST_F(ConfigManagerTest, ParseDurationMonths) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "time_horizon", "1m"));

    auto result = ConfigManager::Load(test_config_path);

    ASSERT_TRUE(result.has_value());
    // 1 month converts to nanoseconds (check it's a reasonable value)
    auto expected = std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::months(1));
    EXPECT_EQ(result->time_horizon, expected);
}

TEST_F(ConfigManagerTest, ParseDurationYears) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "time_horizon", "1y"));

    auto result = ConfigManager::Load(test_config_path);

    ASSERT_TRUE(result.has_value());
    auto expected = std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::years(1));
    EXPECT_EQ(result->time_horizon, expected);
}
...
TEST_F(ConfigManagerTest, ParseDurationInvalidFormatNoUnit) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "min_diff_time", "100"));
...
TEST_F(ConfigManagerTest, ParseDurationInvalidFormatLetters) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "min_diff_time", "abc"));
...
TEST_F(ConfigManagerTest, ParseDurationUnknownSuffix) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "min_diff_time", "100xyz"));
...
TEST_F(ConfigManagerTest, ParseDurationZero) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "time_horizon", "0s"));
...
TEST_F(ConfigManagerTest, ParsePriceInvalidLetters) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "initial_price", "abc"));
...
TEST_F(ConfigManagerTest, ParsePriceInvalidMultipleDots) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "initial_price", "12.34.56"));

    auto result = ConfigManager::Load(test_config_path);

    // std::from_chars parses partially (12.34) and considers it success
    // This is expected behavior of from_chars - it stops at first invalid character
...
TEST_F(ConfigManagerTest, ParseVolumeInvalid) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "min_volume", "not_a_number"));
...
TEST_F(ConfigManagerTest, ParseDoubleInvalid) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "average_trend_value", "1.2.3"));
...
TEST_F(ConfigManagerTest, ParseUint64Invalid) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "steps_count", "abc123"));
...
TEST_F(ConfigManagerTest, ParseNumberWithLeadingWhitespace) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "initial_price", "  100.5  "));
...
TEST_F(ConfigManagerTest, ParsePathWithSpaces) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "price_evolution_path",
                                       "output/my file.csv"));
...
TEST_F(ConfigManagerTest, ValidateInitialPriceNegative) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "initial_price", "-1"));
...
TEST_F(ConfigManagerTest, ValidateInitialPriceNegativeSmall) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "initial_price", "-0.01"));
...
TEST_F(ConfigManagerTest, ValidateInitialPriceZero) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "initial_price", "0"));
...
TEST_F(ConfigManagerTest, ValidateTimeHorizonZero) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "time_horizon", "0ns"));
...
TEST_F(ConfigManagerTest, ValidateTimeHorizonMinimum) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "time_horizon", "1ns"));
...
TEST_F(ConfigManagerTest, ValidateMinDiffTimeGreaterThanMax) {
    ...
    ASSERT_FALSE(result.has_value());
    EXPECT_THAT(result.error(), HasSubstr("min_diff_time must be < max_diff_time"));
}
...
TEST_F(ConfigManagerTest, ValidateMinDiffTimeZero) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "min_diff_time", "0ns"));
...
TEST_F(ConfigManagerTest, ValidateFastEmaZero) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "fast_ema", "0ns"));
...
TEST_F(ConfigManagerTest, ValidateMinVolumeNegative) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "min_volume", "-1"));
...
TEST_F(ConfigManagerTest, ValidateMinVolumeNegativeSmall) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "min_volume", "-0.01"));
...
TEST_F(ConfigManagerTest, ValidateMinVolumeZero) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "min_volume", "0"));
...
TEST_F(ConfigManagerTest, ValidateMaxPositionLessThanMin) {
    ...
    ASSERT_FALSE(result.has_value());
    EXPECT_THAT(result.error(), HasSubstr("max_position must be >= min_position"));
}
...
TEST_F(ConfigManagerTest, ValidateNegativePositions) {
    ...
    ASSERT_TRUE(result.has_value()) << "Negative positions should be valid if min < max";
}
...
TEST_F(ConfigManagerTest, ValidateRejectionProbabilityNegative) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "rejection_probability", "-0.01"));
...
TEST_F(ConfigManagerTest, ValidateRejectionProbabilityNegativeLarge) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "rejection_probability", "-100.0"));
...
TEST_F(ConfigManagerTest, ValidateRejectionProbabilityAbove100) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "rejection_probability", "100.01"));
...
TEST_F(ConfigManagerTest, ValidateRejectionProbabilityAbove100Large) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "rejection_probability", "1000.0"));
...
TEST_F(ConfigManagerTest, ValidateRejectionProbabilityZero) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "rejection_probability", "0.0"));
...
TEST_F(ConfigManagerTest, ValidateRejectionProbabilityMidRange) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "rejection_probability", "50.5"));
...
TEST_F(ConfigManagerTest, ValidateStepsCountZero) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "steps_count", "0"));
...
TEST_F(ConfigManagerTest, ValidateStepsCountOne) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "steps_count", "1"));
...
TEST_F(ConfigManagerTest, ValidateStepsCountLarge) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "steps_count", "999999999999"));
...
TEST_F(ConfigManagerTest, ParseVerySmallPrice) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "initial_price", "0.00000001"));
...
TEST_F(ConfigManagerTest, ParseVeryLargePrice) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "initial_price", "999999999999.99"));
...
TEST_F(ConfigManagerTest, ParseDurationLeadingZeros) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "min_diff_time", "0000100ms"));
...
TEST_F(ConfigManagerTest, PathsWithTrailingSlashes) {
    WriteConfigFile(ModifyConfigValue(GetValidConfigContent(), "price_evolution_path", "output/"));
