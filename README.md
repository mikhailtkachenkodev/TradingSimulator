...
| `average_trend_value` | 0.1 | Коэффициент дрифта (μ) — средняя доходность |
| `price_variation` | 0.15 | Волатильность (σ) — степень случайных колебаний |
| `time_horizon` | 30d | Временной горизонт для нормализации расчётов |
| `min_diff_time` | 50ms | Минимальный интервал между тиками |
...
| `min_volume` | 10 | Минимальный объём ордера |
